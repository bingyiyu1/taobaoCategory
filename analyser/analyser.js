...
const getIsSaleProp = async function (propsFile) {
  ...
  pids.forEach(pid => {
    ...
    pValues.forEach(value => {
      ...
      if (vidJson[value.vid] && vidJson[value.vid] !== name) {
        console.error('value是会有冲突的', name);
      } else {
...
const analysePropsFile = async function (propsFile) {
  const pids = await getIsSaleProp(propsFile);
...
(async () => {
  ...
  await files.map(async file => {
    await analysePropsFile(require(path.join('../data/props', file))); // eslint-disable-line global-require
  });
})();
